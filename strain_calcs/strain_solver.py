...
def calculator( ref_Xaxis, ref_Yaxis, ref_Zaxis, c_d = [11.90e11,5.34e11, 5.96e11], ai = [0.25,0.25,0.25]):
    # Setting up state
    row = 3
    col = 3
    size = (row,col)
    ...
    c_unit_d = np.array([[c_d[0],c_d[1],c_d[1],0,0,0],[c_d[1],c_d[0],c_d[1],0,0,0],[c_d[1],c_d[1],c_d[0],0,0,0],[0,0,0,c_d[2],0,0],[0,0,0,0,c_d[2],0],[0,0,0,0,0,c_d[2]]])
...
axis_vals = [[0, 0, 1], [0, m.sqrt(2), 1], [0, m.sqrt(2), 1] ]
    ...
    for h, y_val in enumerate(axis_vals):
        ...
        for j, z_val in enumerate(axis_vals):
            if j != g and j != h:
                ref_Z= z_val
                # do the calculations
                sol = calculator(ref_X, ref_Y, ref_Z, c)
